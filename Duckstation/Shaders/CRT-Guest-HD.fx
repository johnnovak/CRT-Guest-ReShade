...
#define invsigmah \
        1.0 / (2.0 * SIGMA_HOR * SIGMA_HOR * internal_res * internal_res)

#define invsigmav \
        1.0 / (2.0 * SIGMA_VER * SIGMA_VER * internal_res * internal_res)
...
texture CRTHD_01 < source = "CRT-LUT-1.png";
>
{
    Width  = 1024;
    Height = 32;
...
texture CRTHD_02 < source = "CRT-LUT-2.png";
>
{
    Width  = 1024;
    Height = 32;
...
texture CRTHD_03 < source = "CRT-LUT-3.png";
>
{
    Width  = 1024;
    Height = 32;
...
texture CRTHD_04 < source = "CRT-LUT-4.png";
>
{
    Width  = 1024;
    Height = 32;
...
float3 v_resample(float2 tex0, float4 size)
{
    ...
    do {
        ...
        if (w < 0.0) {
            w = clamp(w, lerp(-maxsharp, 0.0, pow(clamp(fpx, 0.0, 1.0), VSHARP)), 0.0);
        }
...
float corner(float2 pos)
{
    ...
    float2 rs = (bsize == 0.0)
                  ? 1.0.xx
                  : lerp(0.0.xx, 1.0.xx, smoothstep(1.0.xx, 1.0.xx - bc, sqrt(pos)));
...
float4 AfterglowPS(float4 position : SV_Position, float2 texcoord : TEXCOORD)
        : SV_Target
{
    ...
    float3 result = lerp(
        max(lerp(clr, a, 0.49 + float3(PR, PG, PB)) - 1.25 / 255.0, 0.0), clr, w);

    return float4(result, w);
}

float4 PreShaderPS(float4 position
        ...
        : SV_Target
{
    ...
    if (BP > -0.5) {
        color = color + aftrglow.rgb + bp;
    } else {
        color = max(color + BP / 255.0, 0.0) /
                    (1.0 +
                     BP / 255.0 *
                         step(-BP / 255.0, max(max(color.r, color.g), color.b))) +
                aftrglow.rgb;
    }
...
float4 LinearizePS(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    if (interr <= OrgSize.y / yres_div && interm > 0.5 && intres != 1.0 &&
        intres != 0.5 && vga_mode < 0.5 || hscans) {
        ...
        if (interm == 4.0) {
            c = plant(lerp(c, c * c, 0.5 * iscans), max(max(c.r, c.g), c.b)) *
                (1.0 - 0.5 * iscanb);
        }
        if (hscans) {
            c = c1;
    ...
    if (vga_mode > 0.5) {
        c = c1;
        if (interr <= OrgSize.y) intera = 0.75;
        else                     intera = 0.5;
    }

    c = pow(c, gamma_in);

    if (fuxcoord.x > 0.5) gamma_in = intera;
    else                  gamma_in = 1.0 / gamma_in;
...
float4 HGaussianPS(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    do {
        ...
        if (m_glow > 0.5) {
            ...
            pixel = plant(pixel,
                          max(max(max(pixel.r, pixel.g), pixel.b) - m_glow_cutoff,
                              0.0));
        }
...
float4 HD_Pass1_PS(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    float4 PALSize = OrgSize * float4(prescalex.x,
                                      prescalex.y,
                                      1.0 / prescalex.x,
                                      1.0 / prescalex.y);
...
float4 HD_Pass2_PS(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    if ((intres == 0.5 && prescalex.y < 1.5) || vgascan) {
        pC4.y = floor(pC4.y * OrgSize.y) * OrgSize.w + 0.5 * OrgSize.w;
    }
    ...
    if (interb && no_scanlines < 0.05 || hscans && vgascan || hscans) {
        color1 = v_resample(pos,
                            PALSize * float4(1.0, prescaley, 1.0, 1.0 / prescaley));
    }
    ...
    pC4 += dy;
    if ((intres == 0.5 && prescalex.y < 1.5) || vgascan) {
        pC4.y = floor((pos.y + 0.33 * dy.y) * OrgSize.y) * OrgSize.w +
                0.5 * OrgSize.w;
    }
...
float4 ChromaticPS(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    if (shadow_msk > 0.5) {
        ...
        smask = clamp(smask + lerp(smask_mit,
                                   0.0,
                                   min(w3,
        ...
        if (abs(mask_bloom) > 0.025) {
            ...
            if (mask_bloom > 0.025) {
                ...
                cmask = max(lerp(cmask,
                                 cmask * (1.0 - 0.5 * maxbl) +
                                     plant(pow(Bloom, 0.35.xxx), maxbl),
                                 -mask_bloom),
                            cmask);
            }
        }
    ...
    float dark_compensate = lerp(
        max(clamp(lerp(mcut, maskstr, mx), 0.0, 1.0) - 1.0 + mask_compensate, 0.0) + 1.0,
        1.0,
        mx);
    ...
    float vig  = COMPAT_TEXTURE(CRTHD_S2,
                               clamp(pos,
                                     0.0 + 0.5 * OrgSize.zw,
                                     1.0 - 0.5 * OrgSize.zw))
                    .a;
    ...
    if (abs(bloom) > 0.025) {
        ...
        color = pow(pow(color, mask_gamma / gamma_in) +
                        abs(bloom) * pow(Bloom1, mask_gamma / gamma_in),
                    gamma_in / mask_gamma);
    }
